...
import { loadEnv as loadEnv$1, mergeConfig, normalizePath, version, build, createLogger } from 'vite';
...
function electronMainConfigPresetPlugin(options) {
    return {
        ...
        config(config) {
            ...
            defaultConfig.build.rolldownOptions.output['assetFileNames'] = path.posix.join(build.assetsDir || defaultConfig.build.assetsDir, '[name]-[hash].[ext]');
...
function electronMainConfigValidatorPlugin() {
    return {
        ...
        configResolved(config) {
            ...
            const resolvedOutputs = resolveBuildOutputs$1(rolldownOptions.output, libOptions);
            if (resolvedOutputs) {
...
function electronPreloadConfigPresetPlugin(options) {
    return {
        ...
        config(config) {
            ...
            const buildConfig = mergeConfig(defaultConfig.build, build);
            config.build = buildConfig;
            const resolvedOutputs = resolveBuildOutputs$1(config.build.rolldownOptions.output, config.build.lib || false);
            if (resolvedOutputs) {
                const outputs = Array.isArray(resolvedOutputs) ? resolvedOutputs : [resolvedOutputs];
    ...
    return {
        ...
        configResolved(config) {
            ...
            if (!rolldownOptions.input) {
                ...
                throw new Error('build.rollupOptions.input or build.rolldownOptions.input option is required in the electron vite renderer config.');
            }
        }
    };
}
...
const publicAssetPathCache = new WeakMap();
 ...
 */
function modulePathPlugin(factory) {
    const isImportMetaPathSupported = supportImportMetaPaths();
    ...
    return {
        ...
        async load(id) {
            if (id.endsWith('?modulePath')) {
                ...
                const bundles = await bundleEntryFile$1(cleanUrl(id), config);
...
async function bundleEntryFile$1(input, config) {
...
function isolateEntriesPlugin(factory) {
    let entries;
...
async function bundleEntryFile(input, config, shouldLog, preTransformedCount) {
...
let bytecodeId = 0;
function compileToBytecode(code, renderer) {
...
class ConfigFactory {
    ...
    inlineConfig;
    options;
    constructor(baseConfig, inlineConfig, options) {
    ...
    async build(cleanMode) {
        /* eslint-disable-next-line @typescript-eslint/no-explicit-any */
        const config = mergeConfig(deepClone(this.baseConfig), deepClone(this.inlineConfig));
...
class MainConfigFactory extends ConfigFactory {
    processType() {
        return 'main';
    }
    async resolveBuiltinPlugins(config, cleanMode) {
        const configDrivenPlugins = await resolveConfigDrivenPlugins(config);
        return cleanMode
            ? [
...
class PreloadConfigFactory extends ConfigFactory {
    processType() {
        return 'preload';
    }
    async resolveBuiltinPlugins(config, cleanMode) {
...
class RendererConfigFactory extends ConfigFactory {
    processType() {
        return 'renderer';
    }
    async resolveBuiltinPlugins(config, cleanMode) {
...
export { ConfigFactory as C, MainConfigFactory as M, PreloadConfigFactory as P, RendererConfigFactory as R, loadEnv as a, bytecodePlugin as b, resolveHostname as c, defineConfig as d, externalizeDepsPlugin as e, loadConfigFromFile as l, resolveConfig as r, startElectron as s };
